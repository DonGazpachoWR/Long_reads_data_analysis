...
for seq in isoseq masseq; do
    ...
    # Lista para guardar los Job IDs de las descompresiones de esta secuencia
    job_ids=()
    ...
    for condicion in ${lista[@]}; do
        ...
        if [ -f $bed ]; then
            ...
            #Coger Job ID devuelto por Slurm con  --parsable
    ...
    # Ponemos dependencia si ha tenido que generar el .bed
    dep_arg=""
    if [ ${#job_ids[@]} -gt 0 ]; then
        DEP_LIST=$(IFS=:; echo "${job_ids[*]}")
        dep_arg="--dependency=afterok:${DEP_LIST}"

        echo Job de fusión ($seq) con dependencia de Jobs: $DEP_LIST
    else
        echo No hay dependencia. Ejecutando merge para $seq ...
    fi
